...
test("condensed-audio option runs ffmpeg, saves the mp3 and streams it", async ({ page }) => {
  ...
  const action = page.locator(".cc-action", { hasText: "+ condensed audio…" });
...
test("stats page renders totals and per-episode coverage bars", async ({ page }) => {
  ...
  await expect(page.locator(".stats-totals .stat", { hasText: "words known" })).toBeVisible();
  await expect(page.locator(".stats-totals .stat", { hasText: "words added" })).toBeVisible();
