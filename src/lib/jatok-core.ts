// Environment-neutral Japanese-token helpers shared by the Bun server
// (src/lib/jatok.ts) and the web bundle (web/tokenizer.ts).
//
// This module is PURE: it imports nothing and uses only standard JS (regex,
// string ops, Set). It must stay environment-neutral so BOTH the server and the
// browser bundle can include it verbatim — the server keeps the kuromoji loader
// out of here, and the web copy keeps the HTTP dict loader (getTokenizer) in
// web/tokenizer.ts. Both files re-export everything below so every existing
// importer (web/* , src/server/index.ts, tests) is unchanged.
//
// vocabKey/mergeTokens logic MUST stay byte-identical across server and browser
// (server index keys and browser keys would diverge otherwise); keeping a single
// source here is precisely what guarantees that parity.
...
/**
 * Homograph-aware vocabulary key. Single source of truth for both the server
 * index keys and the browser keys — they MUST stay byte-identical or coloring
 * diverges. Key shape:
